...
export default function EloGraphSlide({ data }: { data: UserData }) {
    const ratings = data.eloHistory.map(e => e.rating);
    const peak = Math.max(...ratings, 0);
    const lowest = Math.min(...ratings, 0);
    const isPositive = data.eloChange >= 0;

    const yMin = Math.max(0, lowest - 200);
    const yMax = peak + 50;
    ...
    return (
        <StoryCard id="slide-elo" className={CONTAINERS.slideCard}>
            ...
            <StoryBackground>
                <div className="absolute top-10 left-10 text-white opacity-5"><LineChartIcon size={60} /></div>
                <div className="absolute bottom-20 right-10 text-white opacity-5"><TrendingUp size={50} /></div>
            </StoryBackground>
            ...
            <motion.div className={CONTAINERS.slideContainer} variants={containerVariants} initial="hidden" animate="visible">
                ...
                <motion.div variants={itemVariants} className={TYPOGRAPHY.cardMessage}>
                    <p className={TYPOGRAPHY.comment}>
                        {isPositive ? '"You\'re climbing the ladder!"' : '"A tough year, but you\'re learning."'}
                    </p>
                </motion.div>
            </motion.div>
        </StoryCard>
