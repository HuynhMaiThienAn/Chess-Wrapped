...
// Distinct Palette for Maximum Contrast
const COLORS = [
    '#81b64c', // 1. Brand Green
    '#ffc800', // 2. Brand Yellow (High Contrast)
    '#2c8c99', // 3. Deep Teal (Distinct)
    '#9e5ec2', // 4. Purple (Distinct)
    '#eb6b65', // 5. Soft Red (Distinct)
];
            ...
            <motion.div className={CONTAINERS.slideContainer} variants={containerVariants} initial="hidden" animate="visible">
                ...
                <motion.div variants={itemVariants} className="w-full flex flex-col justify-start items-center px-4 mb-2 z-10">
                    <div className="flex items-center justify-center w-full mb-1">
                        ...
                        <h2 className="text-3xl font-black text-white drop-shadow-md leading-none">Victory<br/>Methods</h2>
                    </div>
                ...
                <motion.div variants={itemVariants} className="w-full h-[260px] relative z-10 -my-2">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
                        ...
                        <div className="text-[9px] text-[#989795] font-bold uppercase truncate max-w-[70px]">
                            {chartData[0].name}
                        </div>
                    </div>
                </motion.div>

                <motion.div variants={itemVariants} className="w-full px-6 mt-4 flex flex-col gap-1.5 z-10 overflow-y-auto max-h-[160px] custom-scrollbar pb-4">
                    {chartData.map((item, idx) => (
                        <div key={item.name} className="flex justify-between items-center text-xs py-1 px-2 bg-[#262421] rounded-md border border-[#3e3c39]">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-white font-bold text-[10px] uppercase tracking-wide">{item.name}</span>
                            </div>
